...
__all__ = ['MethodologySystemPage']


class MethodologySystemPage(Page):
    ...
    def wait_for_loading(self) -> None:
        def condition() -> bool:
            try:
                ...
                assert 'Методолог системы' in self.header.breadcrumbs
                assert 'Справочники' in self.header.breadcrumbs
                assert self.header.directory
                ...
                assert self.navigation.print_forms.visible
                assert self.navigation.notification_settings.visible
